...
# Paramètres SARSA
is_train = True  # True pour l'entraînement, False pour la phase de test
...
render = False  # Activer le rendu uniquement pour la phase de test

# Créer l'environnement sans rendu pour l'entraînement
env_train = gym.make('FrozenLake-v1', map_name="4x4", is_slippery=False, render_mode=None)
...
if is_train:
    ...
    plt.title("Entraînement avec SARSA sur FrozenLake")
    plt.legend()
    plt.show()
...
# Phase de test
    ...
    for episode in range(num_test_episodes):
        ...
        while not done:
            action = epsilon_greedy_policy(state, epsilon=0, training=False)
